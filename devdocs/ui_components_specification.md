# UI Components Specification for C64U Web Control Panel
...
**Usage Notes:**
- Password input ID **must be** `apiPassword` (consistent across all pages)
- Hidden username field helps password managers
- Spinner starts hidden, shown via JS: `$('#spinner').show()`
...
**Usage Notes:**
- First tab has `.active` class, others don't
- First tab content visible, others have `style="display:none;"`
- `data-tab` attribute matches content ID prefix
...
- Labels and inputs flow horizontally
- Button at the end
...
**Usage Notes:**
- Empty placeholder in HTML
- Populate via JS:
  ```javascript
  $('#status-display').html('<span class="success-message">✓ Success!</span>');
  $('#status-display').html('<span class="error-message">✗ Error: ' + msg + '</span>');
  ```
...
**Usage Notes:**
- Starts hidden
- Populate and show via JS:
  ```javascript
  $('#memory-output').text(hexDumpString).show();
  ```

---

## JavaScript Helper Functions (NOT Component Generators)
